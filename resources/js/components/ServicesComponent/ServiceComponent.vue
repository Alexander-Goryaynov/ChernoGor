            ...
            <div v-if="!loading">
              <div v-if="categories">
                <div v-if="isAdmin">
                  ...
                  >
                    Создать категорию <i class="fas fa-plus-circle"></i>
                  </button>
                </div>
                <ul v-for="cat in categories" :key="cat.id + 'cat'">
                  <li>
                    ...
                    <a
                      :id="cat.id"
                      href="#"
                      :class="['mb-1', { 'text-white': active === cat.id }]"
                      @click.prevent="show(cat.id)"
                      >{{ cat.name }}
                    </a>
                  </li>
                </ul>
          ...
          <div class="col-md-8">
            <section class="tabs-content mb-5">
              <article id="tabs-1">
                ...
                <p class="mb-1">
                  На сайте представлен перечень основных предоставляемых услуг.
                  Для получения более подробной информации о каждой категории и
                  подкатегории советуем использовать источник:
                  <a href="https://notariat.ru/sovet/"
                    >https://notariat.ru/sovet/</a
                  >
                </p>
                <div v-if="active !== null">
                  <h4>
                    {{ this.categories.find((x) => x.id === active).name }}
                  </h4>
                  <button
                    v-if="isAdmin"
                    class="action-button float-right mb-1 mr-1"
                ...
                <div v-if="subcategories" class="mt-5">
                  <ul v-for="sub in subcategories" :key="sub.id + 'sub'">
                    ...
                    <li>
                      <a class="text-white mb-2">{{ sub.name }}</a>
                    </li>
                  </ul>
                </div>
                <div v-else>Подкатегорий пока нет...</div>
              </article>
            </section>
          </div>
        </div>
...
export default {
  ...
  methods: {
    show(index) {
      this.active = index;
      if (
        this.categories.find((x) => x.id === index).subcategories.length !== 0
      ) {
    ...
    deleteSub(index) {
      axios
        .delete("/api/v1/subcategories/" + index)
        .then((response) => {
